// Feature: roma-tropicale-pages, Property 14: Social media link accessibility and correctness
...
const socialPlatforms: [string, string][] = [
  ["Instagram", BRAND.socials.instagram],
  ["Spotify", BRAND.socials.spotify],
  ["LinkedIn", BRAND.socials.linkedin],
];

describe("Property 14: Social media link accessibility and correctness", () => {
  it.each(socialPlatforms)(
    "%s link has correct aria-label and href from BRAND.socials",
    (platform, expectedHref) => {
      const { container } = render(<ContactsSection />);
      const link = container.querySelector(`a[aria-label="${platform}"]`);
